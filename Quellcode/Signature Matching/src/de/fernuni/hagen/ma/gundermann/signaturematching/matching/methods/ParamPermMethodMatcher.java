...
/**
 * Dieser Matcher beachtet, dass die Argumenttypen der beiden Methoden in
 * unterschiedlicher Reihenfolge angegeben sein koennen.
 */
public class ParamPermMethodMatcher implements MethodMatcher {
	...
	private MatcherRate getMatchRating(MethodStructure ms1, MethodStructure ms2) {
		...
		Collection<MatcherRate> rates = getMatcherRatingWithPermutedArguments(ms1.getSortedArgumentTypes(),
				ms2.getSortedArgumentTypes(), this::getMatchRatingWithArgumentTypes);

		if (ms1.getSortedArgumentTypes().length > 0 && rates.isEmpty()) {
			return null;
		}
		rates.add(returnTypeRating);
		MatcherRate resultingRate = new MatcherRate(this.getClass().getSimpleName(),
				MethodDelegationRatingFunctions.mRating(rates));
	...
	private Map<Integer[], Class<?>[]> permuteAgruments(Class<?>[] originalArgumentTypes) {
		...
		if (argumentCount > 0) {
			Permuter.permuteRecursiveWithOriginalPositionCached(argumentCount, originalArgumentTypes, orinialPos,
	...
	public Collection<MethodMatchingInfo> calculateMatchingInfos(Method checkMethod, Method queryMethod) {
		...
		Collection<SingleMatchingInfo> returnTypeMatchingInfos = innerTypeMatcherSupplier.get()
				.calculateTypeMatchingInfos(queryMethod.getReturnType(), checkMethod.getReturnType());

		Collection<Map<ParamPosition, Collection<SingleMatchingInfo>>> argumentTypesMatchingInfos = calculateArgumentMatchingInfos(
	...
	private Collection<Map<ParamPosition, Collection<SingleMatchingInfo>>> calculateArgumentMatchingInfos(
			Class<?>[] checkArgs, Class<?>[] queryArgs) {

		Collection<Map<ParamPosition, Collection<SingleMatchingInfo>>> infos = new ArrayList<>();
		Map<Integer[], Class<?>[]> permutations = permuteAgruments(checkArgs);
		if (permutations.isEmpty()) {
			return infos;
		}
		for (Entry<Integer[], Class<?>[]> combination : permutations.entrySet()) {
			Map<ParamPosition, Collection<SingleMatchingInfo>> infoMap = new HashMap<>();
