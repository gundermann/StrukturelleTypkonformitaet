...
public class WrappedAndExactTypeMethodMatcherMatchingInfosTest {
	...
	public void test1() {
		...
		for (MethodMatchingInfo info : matchingInfos) {
			SingleMatchingInfo rtInfo = info.getReturnTypeMatchingInfo();
	...
	public void test5() {
		...
		for (MethodMatchingInfo mmi : matchingInfos) {
			...
			Map<ParamPosition, SingleMatchingInfo> argumentTypeMatchingInfos = mmi.getArgumentTypeMatchingInfos();
			assertThat(argumentTypeMatchingInfos, notNullValue());
			assertThat(argumentTypeMatchingInfos.size(), equalTo(1));
			for (SingleMatchingInfo argMMI : argumentTypeMatchingInfos.values()) {
				...
				Collection<MethodMatchingInfo> argMethodMI = argMMI.getMethodMatchingInfos().values();
	...
	public void test6() {
		...
		for (MethodMatchingInfo info : matchingInfos) {
			...
			Map<ParamPosition, SingleMatchingInfo> argInfos = info.getArgumentTypeMatchingInfos();
			assertThat(argInfos, notNullValue());
			assertThat(argInfos.size(), equalTo(1));

			for (Entry<ParamPosition, SingleMatchingInfo> argEntry : argInfos.entrySet()) {
