...
\hline
\hline
$PROXY ::=$\newline proxy X for PR $\{TARGET\text{*}\}$ & Eine Proxy-Definition besteht aus dem Namen des Proxy-Typs \emph{X}, dem Namen des Typs \emph{PR} für den der Proxy erzeugt wird. \emph{PR} kann dabei der Name eines \emph{required Interfaces} oder der eines \emph{provides Interfaces sein}. Weiterhin besteht ein Proxy aus einer Mengen von Targets, die die Basis für den Proxy bilden. \\
\hline
...
Die Relation $\Rrightarrow_{egsc}$ wird durch die übrigen Matcher in folgender Form beschrieben:
\begin{gather*}
...
Für zwei Typen $A$ und $B$ für die $\matchTyp{A}{content}{B}$ ist die Menge an möglichen Proxytypen wie folgt definiert:}
...
\end{matcherEquivDef}\\
\begin{matcherConvDef}{WrappedTypeMatcher}{
Für zwei Typen $A$ und $B$ für die $\matchTyp{A}{container}{B}$ ist die Menge an möglichen Proxytypen wie folgt definiert:}
Proxy_{content}(A,B) := \left\{ 
				\begin{array}{l|l}
